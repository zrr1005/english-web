...
import {
  getSettings, saveSettings, clearSettings, clearLLMCache,
  getProviderPreset, type LLMSettings
} from '../utils/llm'
...
export default function SettingsPage() {
  ...
  return (
    <div className="max-w-lg mx-auto space-y-8">
      ...
      <div>
        <h1 className="font-display text-3xl font-bold text-ink-700">Settings</h1>
        <p className="text-ink-300 mt-2 text-sm leading-relaxed">
          Connect an LLM API to unlock AI-powered features: subtitle translation, pronunciation tips, and word explanations. Your key stays in your browser.
        </p>
      </div>

      <div className="bg-white rounded-2xl border border-paper-300 shadow-sm p-6 space-y-5">
        <div>
          <label className="block text-xs font-semibold text-ink-400 uppercase tracking-wider mb-1.5">Provider</label>
          <select value={settings.provider} onChange={(e) => handleProviderChange(e.target.value)}
            className="w-full bg-paper-100 border border-paper-300 rounded-xl px-4 py-3 text-sm text-ink-600 focus:outline-none focus:border-amber-300 transition font-sans">
        ...
        <div>
          ...
          <p className="text-xs text-ink-200 mt-1.5">Stored in your browser's localStorage only.</p>
        </div>

        <div>
          <label className="block text-xs font-semibold text-ink-400 uppercase tracking-wider mb-1.5">Endpoint</label>
        ...
        <div>
          <label className="block text-xs font-semibold text-ink-400 uppercase tracking-wider mb-1.5">Model</label>
      ...
      <div className="bg-paper-200 rounded-2xl p-6 space-y-2">
        <h3 className="text-sm font-semibold text-ink-500 mb-3">How to get an API key</h3>
        {[
          { name: 'OpenAI', url: 'platform.openai.com/api-keys', price: 'from $5' },
          { name: 'DeepSeek', url: 'platform.deepseek.com/api_keys', price: '¥1/1M tokens' },
          { name: 'Qwen 通义千问', url: 'dashscope.console.aliyun.com/apiKey', price: 'free tier available' },
        ].map((p) => (
